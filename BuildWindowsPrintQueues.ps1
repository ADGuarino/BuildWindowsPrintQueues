...
$Location = [Microsoft.VisualBasic.Interaction]::InputBox('Enter where the printer will be located', 'Create New Print Queues')
...
#Actual script that creates the print queues
    ...
    foreach ($PrintServer in $PrintServers) {
        Write-Host "Checking if port exists on $PrintServer..."
        ...
        Write-Host "Checking if printer exists on $PrintServer..."
        $Printer = Get-Printer -ComputerName $PrintServer -Name $PrinterName -ErrorAction SilentlyContinue
        If ($Printer.Name -eq $Null) {
        Add-Printer -AsJob -ComputerName $PrintServer -Name $PrinterName -DriverName "$PrintDriver" -PortName $IPAddress -Location $Location -Comment $IPAddress -Shared -ShareName $PrinterName -Published -Verbose
        }
        Else {
